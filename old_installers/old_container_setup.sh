...
check_dependencies() {
    ...
    local DEPS=("whiptail" "curl" "grep" "awk")
    local MISSING=()
    ...
    # Create a progress gauge
    {
        ...
        case "$ARCH" in
            ...
            *) 
                ...
                whiptail --msgbox "❌ Unsupported architecture: $ARCH" 10 60
                return 1
                ;;
        esac
        
        DOCKER_COMPOSE_VERSION="v2.24.2"
        curl -SL "https://github.com/docker/compose/releases/download/${DOCKER_COMPOSE_VERSION}/docker-compose-linux-${ARCH_DL}" \
            -o "$DOCKER_CONFIG/cli-plugins/docker-compose"
        chmod +x "$DOCKER_CONFIG/cli-plugins/docker-compose"
        chown -R "$USERNAME:$USERNAME" "$DOCKER_CONFIG"
        echo 100
...
services:
  database:
    ...
    volumes:
      - hedgedoc_postgres_data:/var/lib/postgresql/data
